// root script file
// Calcula el Rise time de cada pulso
...
void Histo_Rise_Time_pulse(const char* FileName)
{
...
TH2D *AmpvsRTe= new TH2D("AmpvsRT-e", " ", 100,0,0,12,0,0); // Histograma muon
TH2D *AmpvsRTmu= new TH2D("AmpvsRT-mu", " ", 100,0,0,12,0,0); //Histograma Electron
...
for(Int_t i=0; i<NEntries; i++)
 {
    ...
    {
        AmpvsRTe->Fill(RiseTime*2,Amplitud);
    }
    else // Es un muon
    {
        AmpvsRTmu->Fill(RiseTime*2,Amplitud);
    }
   ...
   // EDepvsMaxP->Fit("pol1");
    ...
    AmpvsRTe->Draw();
    a0Canvas->SaveAs("His_AmpvsRTelectrones.pdf");
    AmpvsRTmu->Draw();
    a0Canvas->SaveAs("His_AmpvsRTmuones.pdf");
